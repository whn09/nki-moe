# coding=utf-8
"""
Qwen3 MOE model with NKI kernel optimizations for NXD inference.

This is an optimized version of the Qwen3 MOE model using custom NKI kernels
for improved performance on AWS Trainium2/3 hardware.

NKI Kernels used:
- RMSNorm: Custom NKI RMSNorm kernel for layer normalization
- SiLU: Fused SiLU activation kernel
- Softmax: Optimized softmax for expert routing
- RoPE: Rotary Position Embedding kernel
- Fused Add+RMSNorm: Combined residual add with normalization

For the AWS Trainium2/3 MoE Kernel Challenge - MLSys 2026
"""
...
from nki_kernels import (
    ...
    nki_silu_kernel,
    nki_softmax_kernel,
    nki_fused_add_rmsnorm_kernel,
)
from nki_moe_kernels import (
    NKIExpertRouter,
    NKIExpertMLP,
    nki_fused_expert_mlp_kernel,
    nki_expert_router_kernel,
    nki_expert_aggregation_kernel,
)
from nki_attention_kernels import (
    NKIRoPE,
    NKIQKNorm,
    nki_rope_kernel,
    nki_qk_rmsnorm_kernel,
)
...
# Flag to control NKI kernel usage
_USE_NKI_KERNELS = True


def set_nki_kernels_enabled(enabled: bool):
    """Enable or disable NKI kernel usage."""
    global _USE_NKI_KERNELS
    _USE_NKI_KERNELS = enabled
...
def get_rmsnorm_cls():
    ...
    Returns NKI-accelerated RMSNorm when running on NeuronCore,
    falls back to HuggingFace implementation on CPU.
    """
    if cpu_mode():
        return Qwen3MoeRMSNorm
    elif _USE_NKI_KERNELS:
...
def get_silu_cls():
    """Get the appropriate SiLU activation implementation."""
    if cpu_mode() or not _USE_NKI_KERNELS:
        return torch.nn.SiLU
    else:
        print("Using NKI-accelerated SiLU")
        return NKISiLU


def get_softmax_cls():
    """Get the appropriate Softmax implementation for routing."""
    if cpu_mode() or not _USE_NKI_KERNELS:
        return torch.nn.Softmax
    else:
        print("Using NKI-accelerated Softmax for routing")
        return NKISoftmax
...
class NeuronQwen3MoEAttention(NeuronAttentionBase):
    """
    NKI-optimized attention module for Qwen3 MoE.

    Uses NKI kernels for:
    - Q/K LayerNorm (RMSNorm)
    ...
    def __init__(self, config: Qwen3MoeInferenceConfig):
        ...
        # Override q_layernorm and k_layernorm with NKI RMSNorm
        ...
        # Store config for NKI kernel selection
        self._use_nki_attention = _USE_NKI_KERNELS and not cpu_mode()
        if self._use_nki_attention:
            print(f"NKI-optimized attention enabled for layer with head_dim={self.head_dim}")
...
class NeuronQwen3MoeDecoderLayer(nn.Module):
    """
    NKI-optimized decoder layer for Qwen3 MoE.
    ...
    def __init__(self, config: Qwen3MoeInferenceConfig, layer_idx: int):
        ...
        # NKI optimization flags
        self._use_nki_fused_add_norm = _USE_NKI_KERNELS and not cpu_mode()
        if self._use_nki_fused_add_norm and layer_idx == 0:
            print(f"NKI fused add+rmsnorm enabled for decoder layers")
